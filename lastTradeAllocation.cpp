...
* @Last Modified time: 2020-08-13 10:35:49
*/
...
static vector<double> lastTradeAllocation(int qty, int scale, vector<double> af, vector <double> netPosAcct, int scenario){
    ...
    switch (scenario){
        ...
        case 3: {
            double netPosSystem = accumulate(begin(netPosAcct),end(netPosAcct),0) + scale * qty;
            ...
            // cout<<"********SUM************ "<<accumulate(par.begin(),par.end(),0)<<endl;
            while (accumulate(par.begin(),par.end(),0) != fabs(netPosSystem)){
                if (accumulate(par.begin(),par.end(),0) > fabs(netPosSystem)){
                    double max = -1e9;
                    int index = 0;
                    for(int k=0;k<par.size();k+=1){
                        if(par[k] >= max){
                            index = k;
                            max = par[k];
                        }
